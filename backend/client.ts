      ...
      for await (const event of response.body) {
        ...
        if (event.chunk?.bytes) {
          try {
            ...
            session.hasReceivedResponse = true; // Mark that we've received at least one response
            ...
            try {
              ...
              if (jsonResponse.event?.contentStart) {
                ...
                if (contentEnd?.stopReason === 'INTERRUPTED' || contentEnd?.interrupted === true) {
                  console.log(`Barge-in detected via contentEnd for session ${sessionId}`);
                  this.dispatchEvent(sessionId, 'bargeIn', { interrupted: true });
                }
